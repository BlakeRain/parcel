...
fn add_tailwind_rebuilder<E: IntoEndpoint>(endpoint: E) -> anyhow::Result<impl IntoEndpoint> {
    #[cfg(debug_assertions)]
    {
        ...
        let rebuilder =
            TailwindRebuilder::new(std::env!("CARGO_MANIFEST_DIR"), ["templates", "style"])
...
pub fn create_app(
    ...
    #[cfg(not(debug_assertions))]
    let static_ep = StaticFilesEndpoint::new("./static");

    #[cfg(debug_assertions)]
    let static_ep = StaticFilesEndpoint::new(format!("{}/static", env!("CARGO_MANIFEST_DIR")));
